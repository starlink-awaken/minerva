...
class CostGuard:
    ...
    def check(self, estimated_cost: float) -> bool:
        ...
        if self.current_spend + estimated_cost > self.monthly_budget:
            logger.warning("budget_exceeded", current=self.current_spend, estimated=estimated_cost, budget=self.monthly_budget)
            return False
        return True
...
class ResearchExecutor:
    ...
    async def get_status(self, task_id: str) -> TaskStatus | None:
        ...
        if task_id in self.scheduled_tasks:
            ...
            return TaskStatus(task_id=task_id, status="running" if not t.done() else "completed",
                              mode=ExecutionMode.SCHEDULED, query="", level="", cost=0,
    ...
    async def list_tasks(self, mode: str | None = None) -> list[dict]:
        ...
        for tid, t in self.scheduled_tasks.items():
            tasks.append({"task_id": tid, "mode": "scheduled", "status": "running" if not t.done() else "completed"})
        # Watch
        for tid, t in self.watch_tasks.items():
            tasks.append({"task_id": tid, "mode": "watch", "status": "running" if not t.done() else "completed"})
        if mode:
    ...
    def _log_execution(self, task: ResearchTask, result: ResearchResult, started: str, completed: str):
        ...
        with open(self.state_dir / "execution_log.jsonl", "a") as f:
            f.write(json.dumps(entry) + "\n")

    def _notify(self, task: ResearchTask, result: ResearchResult):
        ...
        # Persist
        with open(self.state_dir / "notifications.json", "a") as f:
