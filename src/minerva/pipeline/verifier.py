...
async def verify_step(llm_client, ctx, stage_name: str) -> dict:
    ...
    try:
        sources = ", ".join(
            sorted({r.get("source", "web") for r in ctx.search_results[:10]})
        ) or "none"
...
async def verify_report(llm_client, report: str, search_results: list[dict]) -> dict:
