...
def live_pipeline_display():
    ...
    def render_stages():
        ...
        return Panel("\n".join(lines) if lines else "[dim]Initializing...[/dim]",
                     title="Stages", border_style="blue")
...
def print_stats_panel(stage_timings: dict[str, float], quality_score: str, source_count: int,
                      entity_count: int, total_time: float, cost: float = 0.0):
    ...
    console.print(table)
    console.print(f"  [dim]Sources: {source_count}[/dim]  |  [dim]Entities: {entity_count}[/dim]  |  "
                  f"[bold]Quality: {quality_score}/100[/bold]  |  "
                  f"[bold yellow]Cost: ${cost:.4f}[/bold yellow]")
    console.print()
