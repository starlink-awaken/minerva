        ...
        if mode in ("semantic", "hybrid"):
            ...
            if not semantic_results:
                ...
                if count_row and count_row[0] > 0:
                    try:
                        import json as _json
                        rows = self.conn.execute("SELECT id, type, name, confidence, properties FROM entities LIMIT 200").fetchall()
                        entity_objs = [Entity(
                            id=r["id"], type=r["type"], name=r["name"],
                            confidence=r["confidence"], properties=_json.loads(r["properties"]),
                        ) for r in rows]
    ...
    async def ingest(self, source: str, source_type: str = "auto") -> dict:
        ...
        if not SQLiteKnowledgeStore._ENTITY_PATTERNS:
            SQLiteKnowledgeStore._ENTITY_PATTERNS = {
                ...
                "Organization": re.compile(r"\b([A-Z][a-z]*\s){0,3}(?:Inc|Corp(?:oration)?|LLC|Ltd|University|Institute|Company)\b"),
                "Date": re.compile(r"\b\d{4}-\d{2}-\d{2}\b|\b(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\w*\s+\d{1,2},?\s+\d{4}\b"),
        ...
        if source_type == "url":
            try:
                ...
                return {"entity_count": 0, "relation_count": 0, "source_path": source, "error": "fetch failed"}
            ...
            if path.exists():
                ...
                return {"entity_count": 0, "relation_count": 0, "source_path": source, "error": "file not found"}
        else:
            return {"entity_count": 0, "relation_count": 0, "source_path": source, "error": f"unsupported type: {source_type}"}
        ...
        if existing:
            return {"entity_count": 0, "relation_count": 0, "source_path": source, "dedup": "skipped"}
        ...
        return {"entity_count": entity_count, "relation_count": relation_count, "source_path": source}
...
class LanceDBVectorStore:
    ...
    async def embed(self, texts: list[str]) -> list[list[float]]:
        ...
        vectors = await loop.run_in_executor(
            None, lambda: model.encode(texts, batch_size=32, show_progress_bar=False)
        )
    ...
    async def search(self, query: str, top_k: int = 20) -> list[dict]:
        ...
        try:
            ...
            return [
                {"id": r.get("id", ""), "content": r.get("text", ""),
                 "similarity": r.get("_distance", 0.0), "metadata": r.get("metadata", {})}
                for r in result
            ]
        except Exception:
            return []

    async def index_entities(self, entities: list[Entity], mode: str = "overwrite"):
        ...
        if not vectors or sum(abs(v) for v in vectors[0]) < 0.001:
            return
        rows = [
            {"id": e.id, "text": texts[i], "vector": vectors[i],
