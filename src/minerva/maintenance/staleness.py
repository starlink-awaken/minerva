...
class StalenessReport:
    ...
    def summary(self) -> str:
        ...
        return (
            f"Found {self.total_stale} stale entries "
            f"({self.critical_count} critical >180d, {self.warning_count} warning >90d) "
            f"out of {self.total_reports_scanned} reports."
        )
...
class StalenessChecker:
    ...
    def _check_file(self, filepath: Path, now: datetime) -> StaleEntry | None:
        ...
        if age_days > self.critical_days:
            reason = f"Critical: {age_days}d old (threshold: {self.critical_days}d)"
        else:
            reason = f"Warning: {age_days}d old (threshold: {self.warn_days}d)"
    ...
    def get_topics_needing_refresh(self) -> list[str]:
        ...
        report = self.scan()
        return [
            f"{e.title} ({e.age_days}d old, {e.source_count} sources)"
            for e in report.stale_entries[:10]
        ]
