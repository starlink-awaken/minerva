...
class GapAnalyzer:
    ...
    def __init__(self, report_dir: str = "~/knowledge/reports"):
    ...
    def _check_source_diversity(self, distribution: dict[str, int]) -> list[Gap]:
        ...
        if academic_ratio < 0.05:
            gaps.append(Gap(
                topic="global",
                gap_type="source_diversity",
                description=f"Low academic source ratio ({academic_ratio:.1%}). Only {distribution.get('academic_paper', 0)} academic papers.",
                severity="MEDIUM",
    ...
    def _check_topic_coverage(
        self, report_files: list[Path], topic_freq: dict[str, int]
    ) -> list[Gap]:
        ...
        if len(single_report_topics) > 3:
            gaps.append(Gap(
                topic="multiple",
                gap_type="depth",
                description=f"{len(single_report_topics)} topics have only a single research report. Deeper follow-up research recommended.",
                severity="LOW",
        ...
        if report_files and len(report_files) > 20:
            ...
            if top_5_topics:
                gaps.append(Gap(
                    topic="trending",
                    gap_type="coverage",
                    description=f"Top 5 most-researched terms: {', '.join(f'{t}({c})' for t, c in top_5_topics)}. Check for diminishing returns.",
                    severity="LOW",
