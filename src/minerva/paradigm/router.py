...
{{"paradigm": "<paradigm_name>", "confidence": 0.0-1.0, "reasoning": "<why>", "alternative": "<second_best>"}}"""


async def classify_paradigm(llm_client, query: str) -> ParadigmResult:
    ...
    try:
        ...
        response = await llm_client.generate(
            system="You classify research questions into paradigms. Output valid JSON only.",
...
def _rule_based_classify(query: str) -> ParadigmResult:
    ...
    if any(w in q for w in ("compare", " vs ", "versus", "which is better", "difference between")):
        return ParadigmResult(paradigm=ResearchParadigm.COMPARATIVE_ANALYSIS, confidence=0.85,
                              reasoning="Detected comparison patterns in query")

    # Problem-solving patterns
    if any(w in q for w in ("why does", "how to fix", "how to solve", "debug", "error", "bug",
                             "optimize", "reduce latency", "improve performance")):
        return ParadigmResult(paradigm=ResearchParadigm.PROBLEM_SOLVING, confidence=0.80,
                              reasoning="Detected problem-solving patterns in query")

    # Policy patterns
    if any(w in q for w in ("policy", "regulation", "impact of", "law", "compliance",
                             "tax", "subsidy", "government", "legislation")):
        return ParadigmResult(paradigm=ResearchParadigm.POLICY_ANALYSIS, confidence=0.80,
                              reasoning="Detected policy/regulation patterns in query")

    # Literature review patterns
    if any(w in q for w in ("survey", "trends", "state of", "literature", "review of",
                             "overview of", "what are the", "latest in", "recent advances")):
        return ParadigmResult(paradigm=ResearchParadigm.LITERATURE_REVIEW, confidence=0.78,
                              reasoning="Detected survey/overview patterns in query")

    # Default: scientific inquiry
    return ParadigmResult(paradigm=ResearchParadigm.SCIENTIFIC_INQUIRY, confidence=0.70,
