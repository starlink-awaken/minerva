...
from minerva.paradigm.types import PARADIGMS, ParadigmResult, ResearchParadigm, VerificationMode
...
async def execute_with_paradigm(
    ...
    for iteration in range(max_iterations):
        ...
        if criteria_met:
            ...
            logger.warning("paradigm_iteration_failed",
                          paradigm=paradigm_def.name,
...
def _check_completion(ctx, paradigm_def, paradigm: ResearchParadigm) -> tuple[bool, list[str]]:
...
def _check_criterion(ctx, criterion: str, paradigm: ResearchParadigm) -> bool:
    ...
    if "counter-argument" in c:
        return any(
            rel.get("counter_argument") and len(rel.get("counter_argument", "")) > 50
            for rel in (ctx.relations or [])
        )
    ...
    if "verification gate" in c:
        for rel in (ctx.relations or []):
            if "quality_score" in rel and rel["quality_score"] >= 60:
                return True
        return False
    ...
    if "hypotheses" in c or "problem" in c or "solution verified" in c:
        has_verification = any(
            rel.get("verification", {}).get("verified", False)
            for rel in (ctx.relations or [])
        )
