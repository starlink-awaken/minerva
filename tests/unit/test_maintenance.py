...
class TestContradictionDetection:
    ...
    def test_contradiction_report_with_findings(self):
        ...
        c1 = Contradiction(
            claim_a="A increases", source_a="s1",
            claim_b="A decreases", source_b="s2",
            topic="test", severity="HIGH",
        )
        c2 = Contradiction(
            claim_a="B is good", source_a="s3",
            claim_b="B is bad", source_b="s4",
            topic="test2", severity="MEDIUM",
        )
...
class TestStalenessChecker:
    ...
    def test_staleness_report_summary(self):
        ...
        from minerva.maintenance.staleness import StalenessReport, StaleEntry
        e1 = StaleEntry(
            path="/tmp/old.md", title="Old", last_updated="2025-06-01",
            age_days=200, reason="Very old", source_count=3,
        )
...
class TestGapAnalyzer:
    ...
    def test_gap_report_summary(self):
        ...
        g1 = Gap(
            topic="t1", gap_type="source_diversity",
            description="Low academic ratio", severity="MEDIUM",
            recommendation="Add academic sources",
        )
        g2 = Gap(
            topic="t2", gap_type="depth",
            description="Single report", severity="LOW",
