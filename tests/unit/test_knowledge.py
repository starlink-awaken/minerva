...
class TestKnowledgeStore:
    ...
    async def test_timeline(self, store):
        ...
        await store.upsert_entity(Entity(
            id="event-1", type="Event", name="Transformer published",
            properties={"date": "2017-06-12"}, valid_from="2017-06-12"
        ))
        await store.upsert_entity(Entity(
            id="event-2", type="Event", name="GPT-3 released",
            properties={"date": "2020-06-11"}, valid_from="2020-06-11"
        ))
        await store.upsert_relation(Relation(
            id="r1", subject_id="event-1", predicate="PRECEDES", object_id="event-2"
        ))
    ...
    async def test_contradictions(self, store):
        ...
        await store.upsert_relation(Relation(
            id="cr1", subject_id="claim-a", predicate="CONTRADICTS", object_id="claim-b"
        ))
