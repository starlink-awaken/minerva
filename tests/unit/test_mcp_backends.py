...
class TestCallMcpTool:
    ...
    async def test_call_mcp_tool_success_returns_text(
    ...
    @patch("minerva.search.mcp_backends.ClientSession")
    @patch("minerva.search.mcp_backends.sse_client")
    async def test_call_mcp_tool_None_when_no_content(
        self, mock_sse_client, mock_client_session
    ):
    ...
    @patch("minerva.search.mcp_backends.ClientSession")
    @patch("minerva.search.mcp_backends.sse_client")
    async def test_call_mcp_tool_returns_None_on_exception(
        self, mock_sse_client, mock_client_session
    ):
