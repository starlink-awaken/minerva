...
class TestRuleEngine:
    ...
    def test_evaluate_entity_passes(self):
        ...
        entity = {
            ...
            "confidence": "HIGH", "last_verified": datetime.now(timezone.utc).isoformat(),
    ...
    def test_evaluate_low_confidence_no_source(self):
        from minerva.knowledge.rules import RuleEngine
        engine = RuleEngine()
        entity = {"id": "e2", "name": "Unknown", "type": "Concept",
                  "confidence": "LOW", "source_ids": []}
        results = engine.evaluate_one(entity)
    ...
    def test_evaluate_stale_entity(self):
        ...
        entity = {"id": "e3", "name": "Old", "last_verified": stale_date,
                  "confidence": "MEDIUM", "source_ids": ["src"]}
        results = engine.evaluate_one(entity)
        failures = [r for r in results if not r.passed]
        assert len(failures) > 0

    def test_rule_and_logic(self):
        from minerva.knowledge.rules import RuleEngine, Rule, RuleCondition, RuleSeverity
        engine = RuleEngine()
        engine.rules = []  # Clear defaults
        engine.add_rule(Rule(
            name="test_and", description="Both conditions must match",
            conditions=[
                RuleCondition("confidence", "eq", "LOW"),
                RuleCondition("source_ids", "empty"),
            ],
            logic="AND", severity=RuleSeverity.WARNING,
        ))
    ...
    def test_rule_or_logic(self):
        ...
        engine.rules = []
        engine.add_rule(Rule(
            name="test_or", description="Either condition triggers",
            conditions=[
                RuleCondition("confidence", "eq", "LOW"),
                RuleCondition("name", "eq", "TestFlag"),
            ],
            logic="OR", severity=RuleSeverity.WARNING,
        ))
    ...
    def test_run_maintenance_rules(self):
        ...
        entities = [
            {"id": "e1", "name": "A", "confidence": "HIGH", "source_ids": ["s1"],
