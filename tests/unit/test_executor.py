...
class TestResearchExecutor:
    ...
    async def test_executor_task_dataclasses(self):
        ...
        from minerva.executor.executor import (
            ResearchTask, ResearchResult, ExecutionMode, TaskStatus,
        )
        ...
        ctx = ResearchContext(query="test", level=ResearchLevel.L2, triage=TriageResult(
            level=ResearchLevel.L2, scores={"goal": 0.9, "constraints": 0.8, "criteria": 0.7},
            cost_estimate=0.3, model_plan={}, search_plan={}, total_score=0.8,
        ))
        ...
        ts = TaskStatus(
            task_id="test-001", status="running",
            mode=ExecutionMode.IMMEDIATE, query="test", level="L2", cost=0.0,
