...
class TestDaemonIntegration:
    ...
    async def test_scheduled_task_creation(self):
        ...
        from minerva.executor.executor import (
            ResearchExecutor, ResearchTask, ExecutionMode, CostGuard,
        )
        ...
        task = ResearchTask(
            id="cron-test-1", query="Daily AI news", mode=ExecutionMode.SCHEDULED,
            level="L1", max_cost=0.0, cron_expr="0 8 * * *",
        )
    ...
    async def test_cost_guard_persists_across_restarts(self):
        ...
        ledger = tmp.NamedTemporaryFile(suffix=".jsonl", delete=False).name
