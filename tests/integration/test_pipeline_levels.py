...
class TestPipelineLevels:
    ...
    def _make_mock_search(self):
        ...
        search.search = AsyncMock(return_value=[
            SearchResult(title="Source A", url="http://a.com", snippet="Content A", source="scholar", rank_score=0.9),
            SearchResult(title="Source B", url="http://b.com", snippet="Content B", source="arxiv", rank_score=0.8),
            SearchResult(title="Source C", url="http://c.com", snippet="Content C", source="web", rank_score=0.7),
            SearchResult(title="Source D", url="http://d.com", snippet="Content D", source="ddg", rank_score=0.6),
            SearchResult(title="Source E", url="http://e.com", snippet="Content E", source="metaso", rank_score=0.5),
        ])
    ...
    def _make_mock_nlp(self):
        nlp = MagicMock()
        ent1 = MagicMock(); ent1.label_ = "ORG"; ent1.text = "OpenAI"
        ent2 = MagicMock(); ent2.label_ = "PERSON"; ent2.text = "Sam Altman"
        doc = MagicMock(); doc.ents = [ent1, ent2]
    ...
    async def test_l0_pipeline_runs(self):
        ...
        triage = TriageResult(level=ResearchLevel.L0, scores={}, cost_estimate=0,
                              model_plan={}, search_plan=[], warnings=[], total_score=0)
    ...
    async def test_l1_pipeline_runs(self):
        ...
        triage = TriageResult(level=ResearchLevel.L1, scores={}, cost_estimate=0,
                              model_plan={}, search_plan=[], warnings=[], total_score=0)
    ...
    async def test_l2_pipeline_runs(self):
        ...
        triage = TriageResult(level=ResearchLevel.L2, scores={}, cost_estimate=0.3,
                              model_plan={}, search_plan=[], warnings=[], total_score=0)
    ...
    async def test_l3_pipeline_runs(self):
        ...
        triage = TriageResult(level=ResearchLevel.L3, scores={}, cost_estimate=2.0,
                              model_plan={}, search_plan=[], warnings=[], total_score=0)
    ...
    async def test_l4_pipeline_runs(self):
        ...
        triage = TriageResult(level=ResearchLevel.L4, scores={}, cost_estimate=5.0,
                              model_plan={}, search_plan=[], warnings=[], total_score=0)
...
class TestSophiaE2E:
    ...
    def test_learner_record_and_retrieve(self):
        ...
        try:
            ...
            learner.record(ResearchTrace(query="Test", paradigm_name="Adaptive",
                                         operations=["decompose","search"], quality_score=85))
